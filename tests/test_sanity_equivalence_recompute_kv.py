...
def _tiny_alternating_sanity_config():
    ...
    + one mlp-only depth). `attn_expert_mode='per_head_recompute_kv'` is
...
def test_sanity_equivalence_gradients_per_param_pair():
    ...
    match (mod the dtype/recompute_kv reshape applied in
    ...
    for pair in tracked:
        ...
        # Use a slightly looser tolerance because the recompute_kv
