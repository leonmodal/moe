...
For workloads that need more than 8 GPUs, use multi-node with `@modal.experimental.clustered`. This gang-schedules multiple containers and connects them via RDMA (up to 3,200 Gbps). See the `modal-experiment` skill for details.
...
For sandboxes, the GPU is set via the `_SANDBOX_GPU` env var:

```bash
_SANDBOX_GPU=H100 python -m modal run gpu-debugging/tools/gpu_sandbox.py \
  --key-path ~/.ssh/id_ed25519.pub --gpu H100
```
