...
```
Parent Agent
  ├── modal deploy gpu-debugging/tools/gpu_sandbox.py    (ONE-TIME — shared by all agents)
...
All sandboxes launched via the `gpu-debugging` skill share the same volume (`gpu-sandbox-workspace`). Agents can coordinate by writing results for others to read:
