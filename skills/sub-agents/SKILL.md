...
modal deploy gpu-debugging/tools/gpu_sandbox.py
...
Each agent gets its own GPU sandbox via the `gpu-debugging` skill's sandbox tool, SSHes in, and works interactively.
...
- For the GPU sandbox primitive: use the `gpu-debugging` skill
- For training/experiment apps to deploy: use the `modal-experiment` skill
